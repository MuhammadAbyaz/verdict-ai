...
export const Card = ({
  ...
  // Create audio element with useAudio hook
  ...
  const handleClick = useCallback(() => {
    ...
    if (audioSrc) {
      // Play the audio directly through the HTML element
    ...
    onClick();
  }, [disabled, onClick, audioSrc, controls, shortcut]);
  ...
  return (
    ...
    >
      {/* Render the audio element from useAudio hook */}
      {audio}

      {/* Add a backup audio element with an ID we can reference */}
      {audioSrc && (
        <audio id={`audio-${shortcut}`} src={audioSrc} preload="auto" />
      )}
